...
class Router
  ...
  def run(organizations_controller, tickets_controller)
    puts '           --             '
    ...
    while @running
      ...
      print `clear`
      route_menu_selection(menu_selection, organizations_controller, tickets_controller)
    end
  end

  private

  def route_menu_selection(menu_selection, organizations_controller, tickets_controller)
    case menu_selection
    when 1 then @users_controller.search(organizations_controller, tickets_controller)
